...
export class Graph {
    ...
    constructor(canvas: HTMLCanvasElement) {
		...
		this.resizeObserver = new ResizeObserver((entries) => {
			for (const entry of entries) {
				if (entry.target == this.canvas) {//entry.contentBoxSize
					this.fixSize();
				}
			}
		});
	...
	// Check if a point is on (or close to) x or y axis
	...
	// Check if a point is on any line
	...
	// Get the closest point which is on a line
    ...
    private drawGrid() {
        ...
        for (let i = gridOffsetX, k = 0; i < this.width; i += tileWidth, k++) {
            ...
            if (this.options.axisNumbers) {
                ...
                this.drawText(this.convert(startx + k * xGrid.scale), i, textPos, false);
            }
        }
        for (let i = gridOffsetY, k = 0; i < this.height; i += tileHeight, k++) {
            ...
            if (this.options.axisNumbers) {
                ...
                this.drawText(this.convert(starty - k * yGrid.scale), textPos, i, false, alignRight ? 'right' : 'left');
            }
        }
        //this.drawAxes();
        this.ctx.fillStyle = oldFill;
	...
	// Get scale number scale based on zoom
    ...
    private drawText(value: string, posX: number, posY: number, background: boolean = false, textAlign: CanvasTextAlign = 'left') {
        const oldFill = this.ctx.fillStyle;
	...
	// Convert numbers to upper index
    ...
    private drawLines() {
        ...
        let drawLabel: { name: string, x: number, y: number, xpos: number, ypos: number } | undefined;
        for (const lineKey in this.lines) {
            const line = this.lines[lineKey];
            if (!line || !line.on) continue;
            this.ctx.beginPath();
            ...
            for (const point of line.points) {
                ...
                if (point.selected) {
                    ...
                    this.ctx.moveTo(x, y);
                    drawLabel = {name: line.name, x: point.x, y: point.y, xpos: x, ypos: y};
                }
        ...
        if (drawLabel) {
            this.drawText(`${drawLabel.name} [${this.convert(drawLabel.x)}, ${this.convert(drawLabel.y)}]`, drawLabel.xpos + 5, drawLabel.ypos - 5, true);
        }
    }
